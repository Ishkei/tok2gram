...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(os.path.join(PROJECT_ROOT, "data", "logs", "run.log"))
    ]
)
...
async def process_creator(creator_config: dict, settings: dict, state: StateStore, uploader: TelegramUploader, cookie_manager: CookieManager, shutdown_event: asyncio.Event):
    ...
    for post in sorted_posts:
        ...
        # Download
        media = download_post(post, os.path.join(PROJECT_ROOT, "data", "downloads"), cookie_content=cookie_content)
        if not media:
...
async def main():
    ...
    try:
        ...
        creators = load_creators(os.path.join(PROJECT_ROOT, "config", "creators.yaml"))
        settings = config.get('settings', {})
        
        logger.info(f"Loaded config and {len(creators)} creators.")
        
        state = StateStore(os.path.join(PROJECT_ROOT, "data", "state.db"))
        uploader = TelegramUploader(
            token=config['telegram']['bot_token'],
            chat_id=settings.get('telegram_chat_id')
        )
        
        cookie_manager = CookieManager(os.path.join(PROJECT_ROOT, "data", "cookies"))
