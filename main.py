...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(os.path.join(BASE_DIR, "logs", "run.log"))
    ]
)
...
async def main():
    ...
    try:
        ...
        state = StateStore(os.path.join(BASE_DIR, "data", "state.db"))
        uploader = TelegramUploader(
            token=config['telegram']['bot_token'],
            chat_id=settings.get('telegram_chat_id')
        )
        
        cookie_manager = CookieManager(os.path.join(BASE_DIR, "data", "cookies"))
